...
dirname = 'X:\Justin\VR36'
rawfname = 'VR36_231007_194718'


#####
lfp_filename = os.path.join(dirname,'lfp.npy')
#lfpts_filename = os.path.join(dirname,'lfpts.npy')
digIn_filename = os.path.join(dirname, 'digIn.npy')
analogIn_filename = os.path.join(dirname, 'analogIn.npy')
analog_in = None
dig_in = None
amp_data_mmap = None
amp_ts_mmap = None
...
for i, filename in enumerate(files):
    if i==0:
        ...
        arr1 = np.memmap(filename[:-4]+'_VC.bin', dtype='int16', mode='w+', shape=amp_data_n[:256,:].T.shape)
        arr1[:] = amp_data_n[:256,:].T
        del arr1
        arr2 = np.memmap(filename[:-4]+'_PPC.bin', dtype='int16', mode='w+', shape=amp_data_n[256:,:].T.shape)
        ...
        amp_data = np.array(amp_data - 32768, dtype=np.int16)
        arr1 = np.memmap(filename[:-4]+'_VC.bin', dtype='int16', mode='w+', shape=amp_data[:256,:].T.shape)
        arr1[:] = amp_data[:256,:].T
        del arr1
        arr2 = np.memmap(filename[:-4]+'_PPC.bin', dtype='int16', mode='w+', shape=amp_data[256:,:].T.shape)
        arr2[:] = amp_data[256:,:].T
        del arr2
        amp_data = np.multiply(0.195,  amp_data, dtype=np.float32)
        print("REAL FS = " + str(1./np.nanmedian(np.diff(ts))))
        dig_in = np.array(np.concatenate((dig_in, digIN)), dtype='uint8')
        analog_in = np.concatenate((analog_in, analogIN[0]), dtype=np.float32)
        amp_data = np.array(amp_data, dtype=np.float32)
        size = amp_data.shape[1]
        startind = np.where(ts>=starts)[0][0]
        ind = np.arange(startind,size,subsamplingfactor)
        amp_data = np.apply_along_axis(decimateSig,1,amp_data[:,startind:])
        amp_data = np.apply_along_axis(decimateSig2,1,amp_data)
        ts = ts[ind]
        fs = fs/float(subsamplingfactor)
        amp_data_mmap = np.concatenate((amp_data_mmap, amp_data), 1)
