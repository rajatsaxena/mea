 function [amplifier_data_AL, amplifier_data_HC] = read_Intan_RHD2000_file_int_dualProbe(filename)
...
if (data_present)
    ...
    %amplifier_data =
    %int16(amplifier_data);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %amplifier_data = 0.195 * (amplifier_data - 32768); % units = microvolts
    
    
%     amplifier_data = im2int16(amplifier_data);
...
%%%%% edited by Rajat 04-10-2021
% filename(end-3:end) = '.bin';
% fname = strcat(filename(1:end-4), '_AL', filename(end-3:end));
% fid_al = fopen(fname,'w');
% fname = strcat(filename(1:end-4), '_HC', filename(end-3:end));
% fid_hpc = fopen(fname,'w');
numChans = length(amplifier_channels);
amplifier_data = amplifier_data - 32768;
amplifier_data = int16(amplifier_data);
amplifier_data_AL = amplifier_data(1:128,:);
% fwrite(fid_al,amplifier_data,'int16');
% fclose(fid_al);
amplifier_data_HC = amplifier_data(129:numChans,:);
% fwrite(fid_hpc,amplifier_data,'int16');
% fclose(fid_hpc);
