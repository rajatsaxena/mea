...
% 5.34 for TR8, 5.84 for TR10, 5.91 for TR11 (uneven, match to RH)
...
% ksdir='Y:\Winny\Data\TR8_2-22-2023\TR8_S1_SpikeSorted_N=2_Batch192_RS_DONE';
...
for c=1:length(goodclusterID) % 140 S1 units for TR8
...
load('channel_numbers.mat'); % chnum, correct channel numbers (index starts from 0), this will be less than 255 if you disable channels in Kilosort
...
% cd 'Y:\Winny\Data\TR8_2-22-2023'; % directory for TR8 lfp data
% alllfp=double(readNPY('TR8_lfp.npy')); % load in lfp data
lfp=alllfp(1:256,:); % only take S1 which is first 256ch 
...
lfp_beh=lfp(:,indices); % for all 256 channels in S1, for a time snippet


% Next, need to sort LFP channels by depth and shank
% Split by shank first
% for 256F S1 contacts are facing away in RH, so shank 1 (1 to 128) is most medial  
% SHANK 1
s1ind=find(channel_positions(:,1)<=20); % use xcoords to find most medial S1 shank, only 101 ch, rely on x position (NOT Channel map...
...
sh1_lfp=lfp_beh(sh1(:,1)+1,:); % medial shank, this index needs to be CHAN NUM from 0 to 127 *** but need to +1 for index, can't index from 0
% in order from 0 to 127

% SHANK 2
s2ind=find(channel_positions(:,1)>20); % indices for lateral shank
sh2=[];
sh2(:,1)=chnum(s2ind,:); % 1st col is channel num
sh2(:,2:3)=channel_positions(s2ind,:); % 2nd col is X and 3rd col is Y (depth)
sh2_lfp=lfp_beh(sh2(:,1)+1,:); % lateral shank, this index is CHAN NUM, need to +1 for index because it starts at 0
...
%     plot(lfpts_beh(1000*1:1000*2),sh1_lfp_sorted(i,[1000*1:1000*2])-(i*500)); hold on; % sup lfp on top
% end

templates=double(readNPY('templates.npy')); % 3d dimension = ch across both shanks, note this is 205 ch
% [nTemplates, nTimePoints, nTempChannels]
...
use_probe = 1; % choose shank 1 (medial), need to run separately for each shank
lfp_sh = sh1_lfp_sorted; % choose LFP from one shank
% lfp_sh = sh2_lfp_sorted;
...
% use_probe = 2; % choose shank 2 (lateral)
% lfp_sh = sh2_lfp_sorted; % choose LFP from one shank
% Ychannel_positions_sh = 2125-(sh2_sorted(:,3)); % flipped ycoords from either shank 1 or 2
...
S1_boundaries=cellfun(@(x) x(x<=endbound),boundaries_depth,'UniformOutput',false);  

%% 7) Load unitdata and loop through cells to categorize units that are superfical vs deep
% load('TR8_S1unitdata_GLM_8-10-2023_FINAL');
...
for c=1:length(unitdata)
    ...
    if unitdepth<=S1_boundaries{1}(2)
        unitdata(c).region='L1'; % Primary somatosensory area upper limb layer 1
        counter1=counter1+1;

    elseif unitdepth<=S1_boundaries{1}(3)
        unitdata(c).region='L2/3'; % Primary somatosensory area upper limb layer 2/3
        counter2=counter2+1;

    elseif unitdepth<=S1_boundaries{1}(4)
        unitdata(c).region='L4'; % Primary somatosensory area upper limb layer 4
        counter3=counter3+1;

    elseif unitdepth<=S1_boundaries{1}(5)
        unitdata(c).region='L5'; % Primary somatosensory area upper limb layer 5
        counter4=counter4+1;

    elseif unitdepth<=S1_boundaries{1}(6) || unitdepth<=S1_boundaries{1}(7)
        unitdata(c).region='L6'; % Primary somatosensory area upper limb layer 6a and 6b
        counter5=counter5+1;

    elseif unitdepth<=S1_boundaries{1}(8) ||  unitdepth<=S1_boundaries{1}(9) 
        unitdata(c).region='CC'; % Corpus callosum
        counter6=counter6+1;

    elseif unitdepth<=S1_boundaries{1}(10)
        unitdata(c).region='CP'; % Caudoputamen (aka Striatum)
        counter7=counter7+1;
    end
end

sum([counter1,counter2,counter3,counter4,counter5,counter6,counter7])

% not getting any superficial units in TR8 S1 in layer 1 or 2/3
% 0 units in layer 1
% 1 unit in layer 2/3
% 20 units in layer 4
% 84 units in layer 5
% 29 units in layer 6
% 1 units in corpus callosum
% 5 units in caudoputamen
...
cell_counts=[counter1; counter2; counter3; counter4; counter5; counter6; counter7]; 
sum(cell_counts); % 140 S1 cells
idx=find(cell_counts~=0);

pie_labels = {};
figure; p=pie(cell_counts(idx),'%.1f%%');  % p=pie(cell_counts(idx),'%.1f%%'); 
title('S1 cells'); %title('fitGLM');
pText = findobj(p,'Type','text'); 
percentValues = get(pText,'String'); 
pie_labels = {'L1: '; 'L2/3: '; 'L4: '; 'L5: '; 'L6: '; 'CC: '; 'CP: '};
...
% Bar plot is better
X = categorical({'L1'; 'L2/3'; 'L4'; 'L5'; 'L6'; 'CC'; 'CP'});
X = reordercats(X,{'L1'; 'L2/3'; 'L4'; 'L5'; 'L6'; 'CC'; 'CP'});
...
myname='TR8_S1unitdata_8-12-2023_FINAL';
...
S1lfp=lfp_beh(1:128,:);
chmapfullshank1=chmapfull(1:128,:,:);
...
%% Quick test
% Process histology with just taking cells that Kilosort marks as 'good'
tdfread('cluster_info.tsv');
goodclusidx=find(string(KSLabel)=="good");
goodclusterID=cluster_id(goodclusidx); % 0 based, cluster ID
spikecluster=readNPY('spike_clusters.npy'); % cluster ID for each spike
