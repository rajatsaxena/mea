...
# waveform properties
if os.path.exists(os.path.join(dirname,'analyzedMetrics','pooledMetricsAllAnimals.csv')):
    print('Loading file.......')
    maindf = pd.read_csv(os.path.join(dirname,'analyzedMetrics','pooledMetricsAllAnimals.csv'))
else:
    troughToPeak = []
    duration = []
    waveforms = []
    ...
    maindf.loc[(maindf.cellType!='Positive') & (maindf.cellType!='Biphasic') & (maindf.wftroughToPeak<=0.425), 'cellType'] = 'FS'
    maindf.loc[(maindf.cellType!='Positive') & (maindf.cellType!='Biphasic') & (maindf.wftroughToPeak>0.425) &  (maindf.acgRiseTime>6), 'cellType'] = 'WI'
