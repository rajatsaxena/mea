...
def saveLFP(animal_name, dirname, opdirname, subsamplingfactor=24):
    """function to load the rhd files and save them as lfp"""
    lfp_filename = os.path.join(opdirname, animal_name + '_lfp.npy')
    #lfpts_filename = os.path.join(opdirname, animal_name + '_lfpts.npy')
    digIn_filename = os.path.join(opdirname, animal_name + '_digIn.npy')
    analogIn_filename = os.path.join(opdirname, animal_name + '_analogIn.npy')
    analog_in = None
    dig_in = None
    amp_data_mmap = None
    amp_ts_mmap = None
    files = natsorted(glob.glob(os.path.join(dirname,'*.rhd')))
    for i, filename in enumerate(files):
        if i==0:
            ...
            amp_data = np.apply_along_axis(decimateSig,1,amp_data)
            ...
            analog_in = np.concatenate((analog_in, analogIN))
            amp_data = np.array(amp_data, dtype=np.float32)
            size = amp_data.shape[1]
            try:
